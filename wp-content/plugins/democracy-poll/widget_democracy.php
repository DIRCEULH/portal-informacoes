...
class widget_democracy extends WP_Widget {
	...
	public function __construct(){
		...
		parent::__construct(
			...
			, __('Democracy Poll','dem') // Name
			, array( 'description' => __('Democracy Poll Widget','dem') ) // Args
		);
	}
	...
	public function form( $instance ){
		...
		$title     = isset( $instance['title'] )           ? esc_attr( $instance['title'] ) : __('Poll','dem');
		...
		<p>
			<label>
				...
				<small><?php _e('Poll question = widget title?','dem')  ?> </small>
			</label>
		</p>

		<p class="demTitleWrap" <?php echo $title_style ?>>
			<label><?php _e('Poll title:','dem'); ?>
		...
		$options = '
		<option value="0">'. __('- Active (random all active)','dem') .'</option>
		<option value="last" '. selected( $show_poll, 'last', 0) .'>'. __('- Last open poll','dem') .'</option>
		<option disabled></option>
		';

		$qu = $wpdb->get_results("SELECT * FROM $wpdb->democracy_q ORDER BY added DESC LIMIT 70");
		foreach( $qu as $quest ){
			$options .= '<option value="'. $quest->id .'" '. selected($show_poll, $quest->id, 0) .'>'. esc_attr($quest->question) .'</option>';
		}

		echo '
		<p>
			<label>'. __('Which poll to show?','dem') .'
