/*jslint browser: true, devel: true, white: true, eqeq: true, vars: true*/
	...
	Dem.setAnswsMaxHeight = function( $that ){
		...
		if( elHeight > maxHeight ){
			...
			var $overlay  = $('<span class="dem__collapser"><span class="arr"></span></span>').appendTo( $el ),
				__expand  = function(){ $overlay.addClass('expanded').removeClass('collapsed'); },
				__collaps = function(){ $overlay.addClass('collapsed').removeClass('expanded'); },
				timeout;

			// стили в head один раз. Перед всеми стилями для дохлого приоритета
			/*
			var $style = $(
				'<style id="dem__collapser">\
					.dem__collapser{ display:block; cursor:pointer; text-align:center; }\
					.dem__collapser:hover{ color:#FF3311; background:rgba(255, 255, 255, 0.3); }\
					.dem__collapser.expanded{ position:static; height:1em; }\
					.dem__collapser.collapsed{ position:absolute; top:0; bottom:0; right:0; left:0; }\
					.dem__collapser .arr{ display:block; position:absolute; bottom:0; width:100%; font-size:150%; line-height:.5; }\
					.dem__collapser.collapsed .arr:before{ content:"▾"; }\
					.dem__collapser.expanded  .arr:before{ content:"▴"; }\
				</style>'),
				$head = $(document.head);

			if( ! $head.find('#dem__collapser').length ){
				var $head = $(document.head), $_el = []

				if( ! $_el.length ) $_el = $head.find('link[type="text/css"]:first');
				if( ! $_el.length ) $_el = $head.find('style:first');
				if( ! $_el.length ) $_el = $head.find('link[src*=".css"]:first');
