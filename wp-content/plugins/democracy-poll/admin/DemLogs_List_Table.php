...
class DemLogs_List_Table extends WP_List_Table {
	...
	private function bulk_action_handler(){
		...
		if( ! $log_ids = array_filter( array_map('intval', $_POST['logids']) ) ){
			Dem::$i->msg['error'][] = __('Nothing was selected.', 'dem');
			return;
		}
		
		// delete
		if( $action == 'delete_only_logs' )
			Dem::$i->del_only_logs( $log_ids );
		
		// delete with votes
		if( $action == 'delete_logs_votes' )
			Dem::$i->del_logs_and_votes( $log_ids );
    ...
    function prepare_items(){ 
		...
		$ORDER_BY = $orderby ? sprintf("ORDER BY %s %s", esc_sql($orderby), $order ) : '';
	...
	protected function get_bulk_actions() {
		return array(
			'delete_only_logs' => __('Delete logs only', 'dem'),
			'delete_logs_votes' => __('Delete logs and votes', 'dem'),
		);
	}
	...
	function table_title(){
		if( $this->poll_id ){
			...
			echo '<h2><small>'. __('Poll\'s logs: ','dem') .'</small>'. esc_html( $poll->question ) .' <small><a href="'. Dem::$i->edit_poll_url($this->poll_id) .'">'. __('Edit poll','dem') .'</a></small></h2>';
		}
	}
		...
		if( $which == 'top' ){
			...
			<div class="alignleft actions" style="margin-top:.3em;">
				'. ( Dem::$opt['democracy_off'] ? '' : 
					'<a class="button button-small" href="'. esc_url( add_query_arg(array('filter'=>$newfilter?null:'new_answers')) ) .'">'. ($newfilter?' &#215; ':'') . __('NEW answers logs','dem') .'</a>'
				) .'
			</div>';
		}
	}
    ...
    function column_default( $log, $col ){
		...
		elseif( $col == 'ip' ){
			...
			return '<a title="'. __('Search by IP', 'dem') .'" href="'. esc_url( add_query_arg( array('ip'=>$log->ip, 'poll'=>null) ) ) .'">'. esc_html($log->ip) .'</a>';
		}
		elseif( $col == 'ip_info' ){
			...
			if( $log->ip ){
				if( ! $log->ip_info || ( is_numeric($log->ip_info) && (time()-DAY_IN_SECONDS) > $log->ip_info ) ){
					$log->ip_info = Dem::ip_info_format( $log->ip );
		...
		elseif( $col == 'qid' ){
			...
			<div class="row-actions">
				<span class="edit"><a href="'. Dem::$i->edit_poll_url($poll->id) .'">'. __('Edit poll','dem') .'</a> | </span>
				<span class="edit"><a href="'. esc_url( add_query_arg( array('ip'=>null, 'poll'=>$log->qid) ) ) .'">'. __('Poll logs','dem') .'</a></span>
			</div>
			';
		}
		elseif( $col == 'userid' ){
		...
		elseif( $col == 'aids' ){
			...
			foreach( explode(',', $log->aids ) as $aid ){
				...
				$new = Dem::$i->is_new_answer($answ) ? ' <a href="'. Dem::$i->edit_poll_url($log->qid) .'"><span style="color:red;">NEW</span></a>' : '';
