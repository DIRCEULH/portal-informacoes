...
class DemPolls_List_Table extends WP_List_Table{
	...
	public function get_columns(){
		$columns = array(
			...
			'id'         => __('ID','dem'),
			'question'   => __('Question','dem'),
			'open'       => '<span class="dashicons dashicons-yes" title="'. __('Poll Opened','dem') .'"></span>', //,
			'active'     => '<span class="dashicons dashicons-controls-play" title="'. __('Active polls','dem') .'"></span>', //,
			'usersvotes' => '<span class="dashicons dashicons-admin-users" title="'. __('Users vote','dem') .'"></span>',
			'answers'    => __('Answers','dem'),
			'in_posts'   => __('In posts','dem'),
			'added'      => __('Added','dem'),
		);
	...
	function extra_tablenav( $which ){
		if( $which == 'top' ){
			echo '';
				echo $this->search_box( __('Search'), 'dem');
			echo '';
		}
	}
	*/
	...
	public function column_default( $poll, $col ){
		...
		elseif( $col == 'question' ){
			$statuses =
			'<span class="statuses">'.
				($poll->democratic ? '<span class="dashicons dashicons-megaphone" title="'. __('Users can add answers (democracy).','dem') .'"></span>' : '').
				($poll->revote     ? '<span class="dashicons dashicons-update" title="'. __('Users can revote','dem') .'"></span>' : '').
				($poll->forusers   ? '<span class="dashicons dashicons-admin-users" title="'. __('Only for registered user.','dem') .'"></span>' : '').
				($poll->multiple   ? '<span class="dashicons dashicons-image-filter" title="'. __('Users can choose many answers (multiple).','dem') .'"></span>' : '').
				($poll->show_results ? '<span class="dashicons dashicons-visibility" title="'. __('Allow to watch the results of the poll.','dem') .'"></span>' : '').
			'</span>';

			$_has_logs = Dem::$opt['keep_logs'] && $wpdb->get_var("SELECT qid FROM $wpdb->democracy_log WHERE qid = ". intval($poll->id) ." LIMIT 1");

			return $statuses . $poll->question . '
			<div class="row-actions">
				<span class="edit"><a href="'. Dem::$i->edit_poll_url( $poll->id ) .'">'. __('Edit','dem') .'</a> | </span>'.
				( $_has_logs ? '<span class="edit"><a href="'. add_query_arg( array('subpage'=>'logs', 'poll'=> $poll->id), $url ) .'">'. __('Logs','dem') .'</a> | </span>' : '') .
				'<span class="delete"><a href="'. add_query_arg( array('delete_poll'=> $poll->id), $url ) .'" onclick="return confirm(\''. __('Are you sure?','dem') .'\');">'. __('Delete','dem') .'</a> | </span>
		...
		elseif( $col == 'usersvotes' ){
			...
			return $poll->multiple ? '<span title="'. __('voters / votes','dem') .'">'. $poll->users_voted .' <small>/ '. $votes_sum .'</small></span>' : $votes_sum;
		}
		elseif( $col == 'in_posts' ){
		...
		elseif( $col == 'active' ){
			return dem_activatation_buttons( $poll, 'reverse' );
		}
		elseif( $col == 'open' ){
			return dem_opening_buttons( $poll, 'reverse' );
		}
		elseif( $col == 'added' ){
