...
function democracy_activate(){
	...
	// multisite
	if( is_multisite() && count( $sites = wp_get_sites() ) > 0 ){
		foreach( $sites as $site ) {
			switch_to_blog( $site['blog_id'] );

			__democracy_activate();
		}
...
function __democracy_activate(){
	...
	if( ! $wpdb->get_row("SELECT * FROM $wpdb->democracy_q LIMIT 1") ){
		$wpdb->insert( $wpdb->democracy_q, array(
			'question'   => __('What does "money" mean to you?','dem'),
		...
		$answers = array(
			__(' It is a universal product for exchange.','dem'),
			__('Money - is paper... Money is not the key to happiness...','dem'),
			__('Source to achieve the goal. ','dem'),
			__('Pieces of Evil :)','dem'),
			__('The authority, the  "power", the happiness...','dem'),
		);
...
function dem_last_version_up(){
	...
	// обновим css
	Dem::$i->regenerate_democracy_css();
